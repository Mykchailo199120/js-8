...
function setUserInfoCookie(key, value) {
  const encodeValue = encodeURIComponent(`${key}=${value}`);

  const expirationDate = new Date();
  expirationDate.setTime(expirationDate.getTime() + 10 * 10000);

  document.cookie = `userinfo=${encodeValue}; expires=${expirationDate.toUTCString()}; path=/`;

  console.log(`Cookie "userInfo" встановлено з даними: ${key}=${value}`)
